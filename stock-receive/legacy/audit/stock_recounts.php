<?php
$page_title = "Stock Recount Audit Trail - Stock Receive System";
...
// Get stock recount-specific audit trail records
$limit = 50;
$page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
$offset = ($page - 1) * $limit;

$stmt = $conn->prepare("SELECT COUNT(*) as total FROM audit_trail WHERE table_name IN ('stock_recounts', 'stock_recount_items')");
$stmt->execute();
...
$stmt = $conn->prepare("SELECT * FROM audit_trail WHERE table_name IN ('stock_recounts', 'stock_recount_items') ORDER BY changed_at DESC LIMIT ? OFFSET ?");
...
<h2>Stock Recount Audit Trail</h2>
<p>This page shows all changes made to stock recounts.</p>
