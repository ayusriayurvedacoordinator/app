...
// Get invoice-specific audit trail records
$limit = 50;
$page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
$offset = ($page - 1) * $limit;

$stmt = $conn->prepare("SELECT COUNT(*) as total FROM audit_trail WHERE table_name IN ('invoices', 'invoice_items')");
$stmt->execute();
...
$stmt = $conn->prepare("SELECT * FROM audit_trail WHERE table_name IN ('invoices', 'invoice_items') ORDER BY changed_at DESC LIMIT ? OFFSET ?");
...
<p>This page shows all changes made to invoices and invoice items.</p>
